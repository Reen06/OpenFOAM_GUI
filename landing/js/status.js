...
function renderSimulationCards(container, runs) {
    ...
    runs.forEach((run) => {
        ...
        const typeIcon = run.type === 'propeller' ? '🔄 Propeller' : '🌬️ Wind Tunnel';

        cardsHtml += `
            <div class="sim-card ${typeClass} ${isCollapsed ? 'collapsed' : ''}" data-run-id="${runId}" data-type="${run.type}">
                <div class="sim-card-header" onclick="toggleCollapse('${runId}')">
                    <button class="collapse-btn">${isCollapsed ? '▶' : '▼'}</button>
                    <span class="sim-type">${typeIcon}</span>
                    ...
                    <div class="sim-actions" onclick="event.stopPropagation()">
                        <button class="sim-view-btn" onclick="viewSimulation('${run.type}', '${runId}')">👁 View</button>
                        <button class="sim-stop-btn" id="stop-${runId}" onclick="stopSimulation('${run.type}', '${runId}', this)">⏹ Stop</button>
                    </div>
                </div>
...
function viewSimulation(type, runId) {
    // Navigate to the appropriate sub-app with run_id to auto-select and tab to switch to solver
    if (type === 'propeller') {
        window.location.href = `/propeller/?run_id=${runId}&tab=solver`;
    } else if (type === 'windtunnel') {
        window.location.href = `/windtunnel/?run_id=${runId}&tab=solver`;
    }
}

async function stopSimulation(type, runId, buttonEl) {
    if (buttonEl) {
    ...
    try {
        // Call sub-app stop endpoint directly
        let targetUrl;
        if (type === 'propeller') {
            targetUrl = `/propeller/api/run/${runId}/stop`;
        } else if (type === 'windtunnel') {
            targetUrl = `/windtunnel/api/run/${runId}/stop`;
        } else {
