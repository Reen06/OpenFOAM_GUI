...
class MeshLibrary:
    ...
    def add_mesh(self, name: str, rotor_path: Path, stator_path: Path, 
                 project: str = "default", polymesh_source_path: Path = None) -> str:
        """
        Add a mesh pair to the library, optionally with pre-created polyMesh.
        
        Args:
            name: Human-readable name for the mesh
            rotor_path: Path to rotor UNV file
        ...
        # Copy UNV files to library
        rotor_dest = mesh_dir / "rotor.unv"
        stator_dest = mesh_dir / "stator.unv"
        shutil.copy2(rotor_path, rotor_dest)
        ...
        self.metadata["meshes"][mesh_id] = {
            ...
            "source_files": {
                "rotor": str(rotor_dest),
                "stator": str(stator_dest)
            },
            "rotor_size": rotor_dest.stat().st_size,
    ...
    def get_mesh_files(self, mesh_id: str) -> Optional[Dict[str, Path]]:
        """Get paths to mesh UNV files."""
        if mesh_id not in self.metadata["meshes"]:
            return None
        
        mesh_dir = self.library_dir / mesh_id
        return {
            "rotor": mesh_dir / "rotor.unv",
